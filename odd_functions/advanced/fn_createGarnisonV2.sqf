...
*
* Exemple :
* [_zo] call ODDadvanced_fnc_createGarnisonV2
* [_zo, True, False] call ODDadvanced_fnc_createGarnisonV2
*
* Variable publique :
*/
...
if (_action) then {
	...
	_NbGarnison = round((((_human_players + 2 )/ 2) + (_taille / 50) + (4*(_loctype^(1.2))) - (((count _locProx)/ 7) ^ 2) - ((((_heure - 12)^2)/ 48 ) + 3 ) + 4)/2);
