...
if (CurrentMission == 1) then {
	...
	[selectRandom _textRenfort] remoteExec ["systemChat", 0];	//envoie un message de renfort
...

	{
		...
		_groups set[_forEachIndex, _group];	//definie le vl
		
	}forEach _groups;		//pour tous les groupes


	_loc3 = nearestLocations [position _zo, locationType, 3000]; //Recup les location a - de 3000m 
	{
		if (text _x in locationBlkList) then {		//si dans la liste Noir 
			_loc3 deleteAt _forEachIndex;				//on delete la location de notre liste 
		};
	}forEach _loc3;		// /!\ pas compté celle ou on est 
	_loc6 = nearestLocations [position _zo, locationType, 6000]; //Recup les location a - de 6000m 
	{
		if (text _x in locationBlkList) then {		//si dans la liste Noir 
			_loc6 deleteAt _forEachIndex;				//on delete la location de notre liste 
		};
	}forEach _loc6;		// /!\ pas compté celle ou on est 
	_loc36 = _loc6 - _loc3;
	...
	if (count(_loc36) > 0) then { 
...

		{
			...
			_refloc set[_forEachIndex, _rdmloc]; // ne selectionne que des obj avec des route
...

		{
			...
			while {(count nearestTerrainObjects [_pos, ["Rocks","House"], 20] > 0) or (!(isonRoad _pos))} do { 		// si il y a plus de 0 cailloux dans les 10 mettres ou pas sur une route 
				_pos = position _loc getPos [random 300, random 360];				//tire une nouvelles position car on veux pas qu'il spawn dans un cailloux
				if (count (_pos nearRoads 300) > 0) then { 							//si il y a des route a coté 
					_pos = position (selectRandom (_pos nearRoads 300) );			//choisi la route la plus proche
				};
			};
			...
			if(!(("brf_o_ard_uaz" in _group) or ("brf_o_ard_uaz_open" in _group))) then {	// ajoute du personnel dans les vl
				...
				_infG = selectRandom squad;		//choisi une squad
				_pos set [1,(_pos select 1)+ 3];	// la deplace sur le coté 
				
				_infG deleteAt (random (count _infG));	//delete 2 gars 
				_infG deleteAt (random (count _infG));
				
				_inf = [_pos, EAST, _infG] call BIS_fnc_spawnGroup; // spawn la squad
				ZoPIA pushBack _inf;		//Ajoute le groupe a la liste des IA de la missions
			};
			
			sleep 1;

			{
				_x moveInCargo (vehicle (units _g select 0));	// déplace l'unité dans le vl
			}forEach units _inf;	//pour chaque unité 
			...
			_g addWaypoint [_posWp1, 100, 1];	//Waypoint déplacement et dechargement 
			...
			[_g, 2] setWaypointType "SAD";		// WP Seek And Destroy
			
			_inf addWaypoint [_posWp1, 0, 1];	//WP get Out 
			[_inf, 1] setWaypointType "GETOUT";

			_inf addWaypoint [(position _zo getPos [random 200, random 360]), 50, 2]; 
			[_inf, 2] setWaypointType "SAD";	// WP Seek And Destroy
